...
function DetailImg() {
  ...
  const loadPin2 = async () => {
    try {
      //lấy pin theo id
      let data = await axios.get(`http://localhost:3579/api/v1/pin/relate/${id}`);
      setPin(data.data.pin);
      // Người người sở hữu
      let user = await axios.get(
        `http://localhost:3579/api/v1/pin/userCreatePin/${id}`
      );
      setPinOwner(user.data.user[0][0]);
      // người subcriber
      let userIdFollewer  =user.data.user[0][0].id;
      let subcriber = await axios.get(
        `http://localhost:3579/api/v1/subcriber/${userIdFollewer}`
      );
      setSubscriber(subcriber.data.subcriber[0]);
